"""CivitaiFlow product release contract.

This module loads after the feature layers and keeps the visible/runtime release
identity in one late-binding compatibility layer. It also provides critical
inline dimensions so the Forge tab remains usable even on Gradio builds that
ignore nested ``Blocks(css=...)`` styles.
"""
...
if UI:
    ...
    def brand_html():
        ...
        <div class="cf-brand" style="display:flex;align-items:center;gap:12px;min-height:48px;overflow:hidden">
            <div class="cf-brand-mark" aria-hidden="true" style="width:36px;height:36px;min-width:36px;display:grid;place-items:center;color:#f97316">
                <svg viewBox="0 0 24 24" fill="none" style="display:block;width:30px;height:30px;max-width:30px;max-height:30px">
    ...
    def build_civitai_frame(cache_buster=None):
        ...
        return (
            ...
            'style="position:relative;display:block;width:100%;min-width:0;min-height:640px;height:clamp(640px,calc(100vh - 170px),980px);overflow:hidden">'
            f'<iframe src="{UI.CIVITAI_BASE_URL}/{suffix}" '
            'title="Civitai embedded browser" '
            'referrerpolicy="strict-origin-when-cross-origin" '
            'allow="clipboard-read; clipboard-write; fullscreen" '
            'loading="eager" '
            'style="display:block;width:100%;height:100%;min-height:640px;border:0"></iframe></div>'
        )
